 ...
 */
export function buildRedirectsContent(data: RedirectsFile): string {
  ...
  const lines: string[] = [
    ...
    '',
    ...allRedirects.map((r) => `${r.from}  ${r.to}  ${r.statusCode}`),
  ];
  return lines.join('\n') + '\n';
}
 ...
 */
export function writeRedirectsTo(outputDir: string, slug: string): void {
  const data = loadRedirectsFile(slug);
  const content = buildRedirectsContent(data);
